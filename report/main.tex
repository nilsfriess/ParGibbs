...
We consider three variants. In the first, we discretise~\eqref{eq:shiftedlaplace} on the unit square ${[0,1]}^2$ using a five-point finite difference stencil. In the second, we instead discretise via $H^1$ conforming finite elements. The last example also uses finite elements but uses a different underlying grid (see Section~\ref{sec:3dfe} below). For all examples we measure the time to set up the samplers and to generate a fixed number of samples, and study how the execution time changes with increasing nunber of processors. We either fix the total problem size (strong scaling) or we fix the local per-processor problem size (weak scaling).
...
\subsection{2D structured grid (finite difference discretisation)}
For this example, we set up a grid hierarchy with five levels, the coarsest level being of size $257 \times 257$ and the finest level consisting of $4097 \times 4097$ grid points. 

We begin with a comparison of the Hogwild Gibbs sampler and the multicolour Gibbs sampler used as standalone samplers on the finest level. Since we have used a simple 5-point finite difference stencil to discretise~\eqref{eq:shiftedlaplace}, two colours are sufficient to colour the lattice. In Figure~\ref{fig:2d_structured_ss} we plotted the strong scaling speed-up $S(p)$, i.e., we computed 
\begin{equation*}
    S(p) = \frac{T(1)}{T(p)}\,,
\end{equation*}
where $T(p)$ is the execution time on $p$ cores, and in Table~\ref{tab:2d_structured_ss} we provide the strong scaling efficiency defined as
\begin{equation*}
    \text{E(p)} = \frac{S(p)}{p}\,.
\end{equation*}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/strong_scaling_hogwild_4097x4097.eps}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{plots/strong_scaling_gibbs_4097x4097.eps}
    \end{subfigure}
...
We observe that the behaviour of both samplers is very similar although the performance of the multicolour Gibbs sampler seems to drop for 1024 cores. Comparing the absolute timings, we found that the Hogwild sampler is on average about 15\% faster than the multicolour sampler. 


\begin{table}[htbp]
    \centering
    \caption{Strong scaling efficiency for the 2D structured grid example. ``Multicolour'' is abbreviated ``MC'', and ``Hogwild'' is abbreviated ``HW''. By MGMC+XX we mean that the Multigrid Monte Carlo sampler was used with ``XX Gibbs'' as the random smoother.}\label{tab:2d_structured_ss}
    \begin{tabular}{rcccc}
    \toprule
    \# Proc. & \textbf{MC Gibbs} & \textbf{HW Gibbs}  & \textbf{MGMC+MC}  & \textbf{MGMC+HW} \\
    \midrule
    {2}     &  94\% &  95\% && \\
    {4}     &  90\% &  87\% && \\
    {8}     &  86\% &  78\% && \\
