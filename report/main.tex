...
We begin with a weak scaling of the square disc example. In this case, instead of fixing the number of grids in the hierarchy and varying the coarse grid size, we fix the coarse grid size and increase the number of levels with increasing number of cores. The number of levels ranges from two (for the one processor run) up to eight (for the run on 1024 processes). The number of colours required for the matrix colouring are (from coarse to fine)
\begin{equation*}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{plots/ws_mgmc_fe.pdf}
    \caption{Weak scaling efficiency for the  MGMC sampler applied to the square disc example.}
    \label{fig:2d_unstructured_ws}
\end{figure}

We continue to explore the effect of the coarse grid samplers by measuring the setup and sampling times for the toroid example. The grid hierarchy now contains of four levels and the number of colours required to colour the matrix graphs are (again from coarse to fine)
\begin{equation*}
  16,\ 45,\ 57,\ 53\,.
\end{equation*}
